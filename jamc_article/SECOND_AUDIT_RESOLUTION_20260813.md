...
- Added the separate `n=1` two-ray theorem with eventual exact connectivity.
- Added a qualitative moving-level corollary without an assumed rate.
- Proved the model-internal approximation-value rate
  `e(l)-e_infinity=O(l^(-1/2))`, yielding unconditional near-optimal barrier
  rate `O(m^(-1/(n+1)))` for `n>=2`.
...
The repository finding is resolved by the curated public revision. An arXiv v2
