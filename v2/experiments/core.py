...
class EmpiricalObjective:
    ...
    Huber and logistic losses are globally Lipschitz in the scalar prediction;
    ``mse`` denotes half-squared error, which is included for the smooth-loss
    theorem and is not passed to the
    global-Lipschitz DSS certificate.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if loss not in {"huber", "logistic", "mse"}:
            raise ValueError("loss must be 'huber', 'logistic', or 'mse'.")
        if kappa <= 0:
    ...
    def loss_lipschitz(self) -> float:
        ...
        if self.loss == "logistic":
            return 1.0
        raise ValueError("Half-squared error has no global logit-Lipschitz constant.")

    @property
    def loss_smoothness(self) -> float:
        if self.loss in {"huber", "mse"}:
            return 1.0
        return 0.25
