...
\hypersetup{
  pdftitle={Online Resource 1: Supplementary Reproducibility and Proof-Audit Notes},
  pdfauthor={Saveliy Baturin}
}
\setlength{\emergencystretch}{3em}

\title{Online Resource 1: Supplementary Reproducibility and Proof-Audit Notes}
...
\textbf{Article title:} Dimension-Free Loss-Landscape Barrier Decay via Atomic
Self-Sparsification

\textbf{Journal:} Journal of Optimization Theory and Applications

\textbf{Author and corresponding author:} Saveliy Baturin

\textbf{Affiliation:} Independent Researcher, Russia

\textbf{E-mail:} \href{mailto:saveliy.xo@gmail.com}{saveliy.xo@gmail.com}
\end{minipage}
\end{center}

\medskip
\noindent
This online resource provides frozen experimental parameters, implementation
details, path-certificate definitions, and provenance information for the
numerical diagnostics reported in the manuscript.

\section*{Primary v2 diagnostic: fixed-dictionary self-sparsification}

The primary v2 computation is implemented in \texttt{v2/experiments}.  Its
versioned configurations and frozen aggregate records are stored under
\begin{center}
\path{v2/experiments/configs/}\\
\path{v2/results/self_sparsification_gpu/}.
\end{center}
For each
input dimension $n\in\{2,8,32,128\}$, it uses a seeded empirical distribution
of 4096 Gaussian inputs, a width-128 ridge-function teacher, and eight dense
width-256 endpoint dictionaries of atomic mass one.  For every endpoint and
$q\in\{4,8,16,32,64,128\}$, the run draws 2048 fixed-dictionary
compressions.  The scalar loss is the half-squared error
$\mathcal L(y,u)=\tfrac12(u-y)^2$ and $\kappa=0.01$.

The code checks the support and exact-mass invariants for every sampled
coefficient vector, evaluates the exact conditional sampling expectation,
compares it with the independent Monte Carlo mean and analytic smooth bound,
and checks the balanced weight-decay lift.  These finite-distribution records
are mechanism and implementation diagnostics.  They do not estimate the
supremum over a population sublevel and do not provide a barrier lower bound.
...
\begin{enumerate}
\item Two Moons: \texttt{make\_moons} with 1000 samples, noise $0.2$, and random state 42, followed by an 80/20 split and standardization on the training portion. Half-squared error is outside the global-Lipschitz hypothesis of the theorem.
...
This warning applies to \texttt{legacy/dss.py} and the historical notebooks.
The complementary complete-path pipeline in the \texttt{theory\_experiments}
directory removes both mismatches, while the primary self-sparsification
diagnostic is isolated under \texttt{v2/experiments}.  The manuscript's
reported finite path stress test uses the synthetic ridge-teacher design, not
the historical breast-cancer notebook.
