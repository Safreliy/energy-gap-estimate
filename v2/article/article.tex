...
\title{Dimension-Free Loss-Landscape Barrier Decay via Atomic Self-Sparsification}
...
\item We introduce penalty-preserving fixed-dictionary self-sparsification. It yields $O(m^{-1/2})$ moving-level thickening for convex Lipschitz losses; a companion smallest-mass pruning argument gives $O(m^{-1})$ at every fixed level above $e_\infty$, uniformly over arbitrary sublevel endpoints and in every input dimension.
...
\item We define the raw-parameter barrier for ordinary two-layer quadratic weight decay and prove the same fixed- and moving-level rates through a filtered balancing retraction. As a topological interpretation, the barrier bound also controls the merge scale of reduced $H_0$ classes.
\item We give theorem-aligned diagnostics that directly test random self-sparsification and smooth first-order cancellation up to input dimension 128, check the balancing lift, and retain a separate complete-path stress test.
\end{enumerate}
...
\end{equation}
for $P_Y$-almost every $y$, with the same constants $\alpha,H$.
We also assume $\E[\mathcal L(Y,0)]<\infty$.
\end{assumption}
...
\end{equation}
The linear term is integrable.  Indeed, applying
\eqref{eq:holder-descent} to a step of length
$(\abs{\partial_u\mathcal L}/H)^{1/\alpha}$ opposite to the derivative and
...
\begin{proposition}[Sharpness of the fixed-dictionary mechanism]\label{prop:compression-sharpness}
...
$X$ be uniform on $\{\sqrt M e_1,\ldots,\sqrt M e_M\}\subset\R^M$, take the
ReLU atoms $\phi_i(x)=\relu(e_i^\top x)$, and put
...
$\phi_i(\sqrt M e_j)=\sqrt M\,\mathbf 1_{\{i=j\}}$, while
$f(\sqrt M e_j)=M^{-1/2}$.  Every omitted coordinate of a $q$-term
...
\begin{proposition}[Legacy ambient-cube certificate]\label{prop:cube-merge}
